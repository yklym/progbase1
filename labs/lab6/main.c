...
int main()
{
    ...
    const int delay = 100;
    char key = '\0';
    const int pointAverage = rand() % (10) + 1;
    ...
    while (1)
    {
        for (int i = 0; i < ballAverage; i++){
            ...
            for(int j = 0; j < pointAverage; j++){
                ...
                if(points[j].loc.x > ball[i].loc.x){
                    ball[i].speed.x -= points[j].distance.x / pow(points[j].power, 2);
                }
                if(points[j].loc.x < ball[i].loc.x){
                    ball[i].speed.x += points[j].distance.x / pow(points[j].power, 2);
                }
                if(points[j].loc.y > ball[i].loc.y){
                    ball[i].speed.y -= points[j].distance.y / pow(points[j].power, 2);
                }
                if(points[j].loc.y < ball[i].loc.y){
                    ball[i].speed.y += points[j].distance.y / pow(points[j].power, 2);
                }
            ...
            if(ball[i].loc.x + ball[i].radius >= w -1 || ball[i].loc.x - ball[i].radius <= 1){
                ball[i].speed.x = -ball[i].speed.x;
            }
            if(ball[i].loc.y + ball[i].radius >= h - 1 || ball[i].loc.y - ball[i].radius <= 2 ){
                ball[i].speed.y = -ball[i].speed.y;
            }
            if(ball[i].loc.x == 0 && ball[i].loc.y == 0){
                ball[i].speed.x += 3;
                ball[i].speed.y += 3;
            }
        }
        ...
        // for (int i = 0; i < i++){
        //     Canvas_setColorRGB(points[].color.red, points[].color.green, points[].color.blue);
        //     Canvas_strokeCircle(points[].loc.x, points[].loc.y, points[].radius);   
        // }
        //
        Canvas_endDraw();
